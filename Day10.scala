...
position=<-21143,  32061> velocity=< 2, -3>
  ...
  def binaryMinSearch[A](f: Int => A)(implicit cmp: Ordering[A]): Int = {
    def growing(x: Int) = cmp.lt(f(x), f(x+1))
    var step = 50
    var upper = 0
    while (!growing(upper)) {
      step *= 2
    ...
    while (upper-lower > 5) {
      val center = lower+(upper-lower)/2
      if (!growing(center))
        lower = center
      else
        upper = center
    }
    (lower to upper).minBy(f)
  }

  implicit class E_ListPoint(self: List[Point]) {
    def boundingBox: Rectangle = {
      val rect = new Rectangle(self.head)
      for (point <- self.drop(1))
        rect.add(point)
      rect
    }
  }

  implicit class E_Rectangle(self: Rectangle) {
    def area: Int = if (self.isEmpty) 0 else self.width * self.height
  }

  case class Star(x: Int, y: Int, vx: Int, vy: Int) {
    def move(timesteps: Int) = Star(x + vx*timesteps, y + vy*timesteps, vx, vy)
    def pos = new Point(x,y)
  }

  val starRegex = """position=< *(-?\d+), *(-?\d+)> velocity=< *(-?\d+), *(-?\d+)>""".r
  val stars = input.lines.toList.map { case starRegex(x,y,vx,vy) => Star(x.toInt, y.toInt, vx.toInt, vy.toInt) }

  val time = binaryMinSearch(time => stars.map(_.move(time).pos).boundingBox.area)
  val alignedStars = stars.map(_.move(time).pos).toSet
  val bounds = alignedStars.toList.boundingBox
  for (y <- 0 to bounds.height) {
    for (x <- 0 to bounds.width) {
      if (alignedStars.contains(new Point(bounds.x+x,bounds.y+y)))
        print('#')
      else
        print('.')
    }
